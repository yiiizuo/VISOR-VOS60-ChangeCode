...
class Encoder_M(nn.Module):
    def __init__(self,backbone):
        ...
        self.rescon = nn.Conv2d(64, 1024, kernel_size=3, stride=4)
        self.bncon = nn.BatchNorm2d(1024)
        self.rescon2 = nn.Conv2d(1024, 1024, kernel_size=3, padding=1)
        self.bncon2 = nn.BatchNorm2d(1024)
    ...
    def forward(self, in_f, in_m, in_o):
        ...
        rescon = self.bncon(self.relu(self.rescon(x) + r4))
        rescon = self.relu(self.bncon2(self.rescon2(rescon)))
        return rescon, r3, r2, c1, f
...
class Refine(nn.Module):
    def __init__(self, backbone, inplanes, planes, scale_factor=2):
        ...
        self.convHS = nn.Conv2d(planes, inplanes, kernel_size=(3,3), padding=(1,1), stride=1)
        self.ResHS = ResBlock(backbone, inplanes, inplanes)
        self.ResMM1 = ResBlock(backbone, inplanes, planes)
        self.ResMMCon = ResBlock(backbone, planes, planes)
        self.bnmm = nn.BatchNorm2d(planes)
        self.act = nn.ReLU()
        self.scale_factor = scale_factor

    def forward(self, f, pm):
        hpm = self.ResHS(self.convHS(pm))
        hpm = F.interpolate(hpm, scale_factor=self.scale_factor, mode='bilinear', align_corners=False)
        m1 = self.ResMM1(hpm+f)
        s = self.ResFS(self.convFS(f))
        m = s + F.interpolate(pm, scale_factor=self.scale_factor, mode='bilinear', align_corners=False)
        m = self.ResMM(m)
        mc = self.act(self.bnmm(self.ResMMCon(m+m1)))
        # return m
        return mc
