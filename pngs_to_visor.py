...
def sequence_of_pngs_to_visor(sequence_path, mapping):
    ...
    for frame in sequence_frames:
        frame_data = {"image": {"image_path": "", "name": "", "video": ""},
                      "annotations": []}  # to store relevent into of each object in that frame
        ...
        frame_data["image"]["image_path"] = '_'.join(
            os.path.basename(sequence_path).split('_')[:2]) + '/' + os.path.basename(frame)
...
def pngs_to_visor(masks_path, mapping_file, out_json_name):
    ...
    all_json_data = {
        "info": {"Dataset Name": "VISOR", "Challenge": "semi_supervised_vos", "Version": "0.1", "Resolution": "854x480",
                 "Release Date": "Aug 2022", "URL": "https://epic-kitchens.github.io/VISOR",
                 "Details": "This is a VISOR VOS submission file. All annotations must have 854x480 resolution"},
        "video_annotations": []}
    ...
    for sequence in tqdm(sequences):
        if os.path.isdir(sequence):
            ...
            sequence_polygons, sequence_frame_count, sequence_masks_count = sequence_of_pngs_to_visor(sequence,
                                                                                                      json_data_mapping[
                                                                                                          os.path.basename(
                                                                                                              sequence)])
...
if __name__ == "__main__":
    def get_arguments():
        ...
        parser.add_argument("--masks_path", type=str, help="path to where the PNGs(predictions) are stored",
                            default='/root/code/results_png')
        parser.add_argument("--mapping_file", type=str,
                            help="path where the mapping file of your data, this would be saved when you run visor_to_davis.py script",
                            default='/data/EPIC-KITCHENS/DAVIS/VISOR_2022/val_data_mapping.json')
