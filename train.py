...
def get_arguments():
    ...
    parser.add_argument("-train_set_txt", type=str, help="name of the text file that contains the traning sequences",
                        default='train')
    parser.add_argument("-val_set_txt", type=str,
                        help="name of the text file that contains the validation sequences(for evaluation)",
                        default='val')
...
palette = Image.open(
    os.path.join(DATA_ROOT + 'Annotations/480p/P01_01_seq_00001/P01_01_frame_0000000140.png')).getpalette()

torch.backends.cudnn.benchmark = True

Trainset_sparse = VISOR_MO_Train(DATA_ROOT, resolution=resolution, imset='20{}/{}.txt'.format(year, train_set_txt),
                                 single_object=False)
Trainloader_sparse = data.DataLoader(Trainset_sparse, batch_size=1, num_workers=0, shuffle=True, pin_memory=True)
loader_iter_sparse = iter(Trainloader_sparse)

Testloader = VISOR_MO_Test(DATA_ROOT, resolution=resolution, imset='20{}/{}.txt'.format(year, val_set_txt),
                           single_object=False)
# device = torch.device("cuda")
device_ids = [1]
model = nn.DataParallel(STM(args.backbone).cuda(), device_ids=device_ids)
...
for iter_ in range(args.total_iter):
    if (iter_ == 0):
        ...
        print("J&F:", g[0])
        if wandb_logs:
            wandb.log({'iteration': iter_, 'J&F-Mean': g[0], 'J-Mean': g[1], 'J-Recall': g[2], 'J-Decay': g[3],
                       'F-Mean': g[4], 'F-Recall': g[5], 'F-Decay': g[6]})
    ...
    if ((iter_ + 1) % log_iter == 0):
        print('iteration:{}, loss:{},remaining iteration:{}'.format(iter_, loss_momentum / log_iter,
                                                                    args.total_iter - iter_))
        if wandb_logs:
    ...
    if (iter_ + 1) % save_step == 0:
        ...
        torch.save(model.state_dict(), os.path.join(args.save, '{}_{}_{}_{}_{}.pth'.format(args.name, args.backbone,
                                                                                           str(args.total_iter),
                                                                                           str(args.batch),
                                                                                           str(iter_))))
